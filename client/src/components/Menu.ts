...
function Menu({ app, user, back }) {
  ...
  const createMenuBodyHandler = () => {
    switch(this.state.tap) {
      case 0: return çreteSaleBody();
      case 1: return createChatBody();
      case 2: return createHeartBody();
    }
  }
  const çreteSaleBody = ():string => {
    return tmpProductList.reduce((acc, cur) => acc + createProductItem(cur, true), '<div class="product-list">') + '</div>';
  };
  const createChatBody = ():string => {
    return tmpChatList.reduce((acc, cur) => acc + createChatItem(cur), '<div class="chat-list">') + '</div>';
  };
  const createHeartBody = ():string => {
    return tmpProductList.reduce((acc, cur) => acc + createProductItem(cur, false), '<div class="product-list">') + '</div>';
  };
    ...
    return `
      <div class="product-list-item">
        ...
        <div class="product-list-item__content">
          <div class="icon ${sale ? 'icon-more' : 'icon-heart'} product-list-item__heart ${product.heart ? 'active': ''}"></div>
          <p class="product-list-item__title">${product.name}</p>
          <p class="product-list-item__info">${product.region} - ${product.time}</p>
    ...
    <div class="chat-list-item">
      ...
      <div class="chat-list-item__content-right">
        <div class="chat-list-item__info">
          ...
          ${chat.chatCount ? '<div class="chat-badge">'+chat.chatCount+'</div>': ''}
        </div>
  ...
  this.render = () => {
    ...
    rerenderMenuBody();
    const gnbContainer:HTMLDivElement = document.querySelector('.gnb-container');
    gnbContainer?.addEventListener('click', (e) => handleTap(e));
  };

  this.rerender = rerenderMenuBody;

  target.addEventListener('click', (e) => isBackButton(e) && back());
